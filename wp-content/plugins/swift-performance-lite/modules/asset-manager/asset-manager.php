...
class Swift_Performance_Asset_Manager {
	...
	public function asset_manager() {
		if (Swift_Performance_Asset_Manager::should_optimize()){
			...
			add_action('wp_head', function(){
				if (Swift_Performance_Asset_Manager::should_optimize()){
	                        echo '<!--[if swift]>SCRIPT_LATE_PRELOAD_PLACEHOLDER<![endif]-->';
				}
                  }, PHP_INT_MAX);
	...
	public function asset_manager_callback($buffer){
            ...
            $css              	= $lazyload_scripts_buffer = $dns_prefetch = $font_preload = $script_preload = $script_late_preload = $style_preload = $media_preload = $blocked_scripts = array();
		...
		if (Swift_Performance::check_option('use-compute-api', 1) && Swift_Performance::is_feature_available('server_side_script') && Swift_Performance::check_option('server-side-script', '', '!=')){
		...
		foreach ($html->find('link[rel="stylesheet"], style, script, img, iframe') as $node){
			// Collect scripts for Autocomplete
			if ($node->tag == 'script' && isset($node->src) && !empty($node->src)){
				Swift_Performance_Autocomplete::collect('scripts', $node->src);
			}

			// Collect styles for Autocomplete
			if ($node->tag == 'link' && isset($node->href) && !empty($node->href)){
				Swift_Performance_Autocomplete::collect('styles', $node->href);
			}
			...
			if (!empty($block_scripts)){
				...
				if (!$webp_excluded){
					$webp_filename = self::get_webp_filename($original_src);
					if (Swift_Performance::check_option('serve-webp', 'picture') && $webp_filename !== false){
						$img_node = $node;
						if ($lazyloaded){
							$node->outertext = '<picture><source data-swift-image-lazyload="true" data-srcset="' . $webp_filename . '" type="image/webp">'.$img_node->outertext.'</picture>';
						}
						else {
							$node->outertext = '<picture><source srcset="' . $webp_filename . '" type="image/webp">'.$img_node->outertext.'</picture>';
						}
						if (!empty($preload)){
							$preload = '<link rel="preload" href="'.$webp_filename.'" as="image">';
						}
					}
				}
		...
		// Smart Fonts
		if (Swift_Performance::is_feature_available('smart_fonts') && Swift_Performance::check_option('smart-fonts', 1) && !empty($critical_css)){
			$result = Swift_Performance_Pro::smart_fonts($critical_css, $_html, $css_dir);
			if (!empty($result)){
				$critical_css = $result;
			}
		}
		...
		if (Swift_Performance::check_option('cache-empty-minicart', 1)){
			...
			if (Swift_Performance::check_option('async-scripts', 1)){
					...
					$js_content = str_replace('(document).ready(', '(', $js_content);
					...
					if (Swift_Performance::check_option('script-delivery', 'smart')){
						...
						$script_late_preload[] = '<link rel="prefetch" href="'.SWIFT_PERFORMANCE_URI.'js/loader.worker.js'.'" as="script" crossorigin>';
					}
					else {
			...
			if (Swift_Performance::check_option('merge-scripts', 1) && Swift_Performance::check_option('server-push', 1, '!=')){
				$script_late_preload[] = '<link rel="prefetch" href="'.$merged.'" as="script" crossorigin>';
			}
            }
		...
		$_html = str_replace('<!--[if swift]>SCRIPT_LATE_PRELOAD_PLACEHOLDER<![endif]-->', implode("\n", array_filter($script_late_preload)), $_html);
		...
		if (Swift_Performance::check_option('use-compute-api', 1) && Swift_Performance::check_option('preload-fonts', 1) && !empty($critical_css) && Swift_Performance::is_feature_available('preload_fonts')){
      ...
      public function lazyload_images($args, $id){
		if (!isset($args['src']) || preg_match('~^data~', $args['src'])){
			...
			if (file_exists(SWIFT_PERFORMANCE_CACHE_DIR . 'lazyload/' . $placeholder)){
				...
				if (!empty($image_type) && !empty($source)){
					if ($image_type == 2){
	...
	public static function webp_background($matches){
		$webp_filename = self::get_webp_filename($matches[2]);
		...
		if (!$webp_excluded && $webp_filename !== false){
			return 'url(' . $webp_filename . ')';
		}
		return $matches[0];
	}
	...
	public static function get_webp_filename($original_src){
		$webp_path = str_replace(apply_filters('swift_performance_media_host',Swift_Performance::home_url()), ABSPATH, Swift_Performance::canonicalize($original_src)) . '.webp';
		if (file_exists($webp_path)){
			return $original_src . '.webp';
		}
		else {
			$webp_path = preg_replace('~\.(jpe?g|gif|png)\.webp$~', '.webp', $webp_path);
			if (file_exists($webp_path)){
				return preg_replace('~\.(jpe?g|gif|png)$~', '.webp', $original_src);
			}
		}
