	      ...
	      <select class="luv-framework-dropdown-field" name="<?php echo esc_attr($name);?>"<?php echo (isset($field['multiple']) && $field['multiple'] ? ' multiple' : '');?>>
	            <?php if (isset($field['options'])): ?>
	                  <?php foreach ($field['options'] as $key => $option):?>
	                        <option value="<?php echo esc_attr($key);?>"<?php echo ((is_array($field['value']) && in_array($key, $field['value'])) || ((string)$field['value'] == (string)$key) ? ' selected' : ''); ?>><?php echo esc_html((string)$option);?></option>
	                  <?php endforeach;?>
	            <?php endif; ?>
	      </select>
      </div>
